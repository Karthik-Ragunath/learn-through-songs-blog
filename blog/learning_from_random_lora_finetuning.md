...
After trying out ACE-Step v1.5 — an open-source diffusion model that generates full songs from text prompts — I noticed something that kept bugging me. The model produces genuinely impressive music: the melodies work, the instruments sound right, the structure makes sense. But the lyrics? sometimes garbled, mumbled, or sometimes just completely wrong. You give it a verse and what comes out sounds like singing, except none of the words were sometimes the words you asked for.
...
|-------|---------|---------|-----------|
| Baseline (no LoRA) | 56.42 | 49.32 | 0.1284 |
| **+ LoRA** | **49.40** | **41.19** | **0.1426** |

That's a **12.4% relative WER reduction** and a **16.5% relative CER reduction**. The attention alignment also improved by about 11%.
...
To be clear: these numbers are still not great in absolute terms. A WER of 49% means roughly half the words are wrong. But the fact that any improvement happened at all with no lyric data in training is the interesting part.

### Where It Helped Most

The biggest wins came in genres where the baseline produced literally nothing recognizable — WER of 100% (complete silence, instrumental-only output, or gibberish):

| Genre | Baseline WER | LoRA WER | What Changed |
|-------|-------------|----------|--------------|
| Jazz | 1.00 | 0.32 | Went from silence to "Moonlight falls on empty streets, Silver shadows come alive, Every whisper softly meets" |
| Synthwave | 1.00 | 0.14 | Went from "Thank you." to nearly the full lyrics |
| Gospel | 1.00 | 0.40 | Went from silence to 3 out of 4 lines correct |
| Folk | 1.00 | 0.40 | Went from silence to first two lines clear |
| Funk | 1.00 | 0.35 | Went from silence to "Feel the rhythm in your bones, Everybody find your groove" |
| New Wave | 1.00 | 0.28 | Went from a music note emoji to most lyrics correct |
| Celtic | 1.00 | 0.52 | Went from "© transcript Emily Beynon" to recognizable lyrics |
| Country | 0.95 | 0.45 | Went from one garbled fragment to half the lyrics correct |

My interpretation: these are genres where the base model's vocal rendering was so poor that it produced silence or instrumentals instead of attempting vocals. The LoRA adapter, by teaching the model what clean singing sounds like, gave it enough confidence (for lack of a better word) to actually render vocals. And once it renders vocals, the base model's existing lyric conditioning — imperfect as it is — gets a chance to work.

### Where It Hurt

Here's the uncomfortable part. In several genres where the baseline was already good, the LoRA adapter made things worse:

| Genre | Baseline WER | LoRA WER | What Happened |
|-------|-------------|----------|---------------|
| Hip-hop | 0.05 | 0.86 | Went from nearly perfect to barely audible mumbling |
| EDM | 0.05 | 1.00 | Went from correct lyrics to complete silence |
| Rock | 0.04 | 0.30 | Went from near-perfect to truncated output |
| Ambient | 0.05 | 0.65 | Went from correct to garbled |
| Power ballad | 0.00 | 0.13 | Went from perfect to minor errors |

This also makes sense in context. The LoRA adapter was trained exclusively on clean acapella studio vocals — solo singers with no instruments, no effects, no production. It biases the model toward that specific kind of output. For genres like hip-hop and EDM, where the base model already knew how to render vocals in a genre-appropriate way, the adapter pushes things toward a "clean studio vocal" character that conflicts with what those genres need.
...
- **Baseline Celtic:** Whisper heard "© transcript Emily Beynon" (the model produced something that sounded like a credits screen)
- **Baseline Blues:** Whisper returned a music note emoji 🎵 (pure instrumental, no vocals at all)
- **Baseline Choral:** Instead of "Glory rises from the dawn," the baseline produced "Holy Christ is the Lord of all" — it hallucinated entirely different religious lyrics
- **LoRA Lullaby:** "The Disney's Big Big Did Ask To Christmas The Disney and" — the adapter tried but produced something from a children's fever dream

The LoRA R&B result is a clean comparison. The ground truth was "Lay beside me close your eyes / Let the music take us home / Underneath the velvet skies / Never have to be alone." The baseline heard "Baby, save me, close your eyes / Let the music take us home / Underneath the flowers, cause I don't have to be known" — close in rhythm but wrong in words. The LoRA version got the first three lines right ("Lay beside me, close your eyes / Let the music take us home / Underneath the velvet skies") but truncated the last line. Better, despite the adapter never having seen those lyrics during training.
...
Each plot has five panels showing all model variants side by side: Baseline, LoRA (no lyrics), LoRA+ASHR (no lyrics), LoRA with lyrics, and LoRA with lyrics+ASHR. The first three panels relate to the Part 1 experiments discussed in this section. The last two show the Part 2 experiments (training with actual lyrics) — we analyze those panels in detail further below.

### Pop

![Cross-Attention Maps — Pop](figures/attention_pop.png)

The most instructive example. The **Baseline** (left) shows a visible diagonal pattern — you can see attention shifting from "I" / "can" / "feel" in the early frames to "Nothing" / "in" / "the" / "world" in the later frames. There's also a bright horizontal band (an "attractor" token — usually a newline or structural tag — that gets attention at all time positions).
...
Baseline pop WER was 0.15 (already decent), LoRA was 0.25 (slightly worse). Both generated recognizable lyrics. The LoRA adapter didn't universally improve attention patterns — it changed them in ways that sometimes help and sometimes don't.

### Folk: Rescuing a Failed Generation

![Cross-Attention Maps — Folk](figures/attention_folk.png)

Baseline folk WER was 1.00 — Whisper heard complete silence, no vocals at all. The LoRA version achieved 0.40 — it produced "Walking down the morning road, Sunlight breaking through the trees, Every step..." before trailing off.

The attention maps show why this is interesting. The **Baseline** has attention spread across multiple lyric tokens but the model still decided not to render vocals. The **LoRA** version has a different pattern and the model did render vocals. The LoRA adapter changed the model's behavior regarding *whether to sing at all*, not just *how well to align lyrics*. That's consistent with what the adapter actually learned: "when there's a clean vocal description, produce vocals."

### R&B

![Cross-Attention Maps — R&B](figures/attention_rnb.png)

The **Baseline** (left) shows the most structured pattern — a clear diagonal in the upper half, with attention shifting progressively through the lyric tokens. Despite this nice-looking pattern, WER was 0.38 — the model produced wrong words ("Baby, save me" instead of "Lay beside me").

The **LoRA** (center) has a simpler pattern with less visible diagonal structure, yet produced better lyrics (WER 0.29). This tells us that the averaged attention heatmap is a lossy summary — the meaningful changes happen at individual layers and heads, and the averaging washes them out.

### Jazz: From Nothing to Something

![Cross-Attention Maps — Jazz](figures/attention_jazz.png)

The **Baseline** jazz attention shows a single dominant attractor band with minimal other structure. It produced complete silence (WER 1.00). The **LoRA** version has a similar overall pattern with subtle differences, and managed to produce three out of four lyric lines correctly (WER 0.32).

Visually subtle change, dramatic output difference. The LoRA adapter is making small adjustments to how the DiT processes its conditioning, and those small adjustments are enough to flip the model from "don't render vocals" to "render mostly-correct vocals." All this without the adapter ever having seen the words it's now singing.
...
I called this Attention SHarpness Regularization (ASHR). I set λ=0.02 and retrained the LoRA adapter from scratch with this additional loss term.

### What Actually Happened

| Model | WER (%) | CER (%) | AAS (DiT) |
|-------|---------|---------|-----------|
| Baseline | 56.42 | 49.32 | 0.1284 |
| + LoRA | 49.40 | 41.19 | 0.1426 |
| **+ LoRA + ASHR (λ=0.02)** | **86.46** | **74.89** | **0.0509** |

It made everything dramatically worse. WER went from 49% (LoRA) to 86% (LoRA+ASHR). The attention alignment score dropped by 64%. The model essentially stopped producing coherent output.

Looking at the actual outputs across 30 genres:

- **10 out of 30** produced silence or instrumental-only output (Whisper returned empty strings or 🎵)
- **12 out of 30** hallucinated completely unrelated lyrics ("No, no, no, I ain't the man you're looking for" when prompted with a folk verse about nature)
- **8 out of 30** got some words right but truncated early or repeated fragments

Some highlights from the hallucination category:
- **Folk** (should have been "Walking down the morning road"): "No, no, no, I ain't the man you're looking for, I'm the one you're looking for, I'm the one you're looking for..." — stuck in a loop
- **Power ballad** (should have been "Reaching for the highest star"): "eeny, meeny, miny, moe"
- **Synthwave** (should have been "Neon lights are flashing bright"): "Ascht 핵 bird Ascht Hydrode In waiter Distinguished" — multilingual gibberish
- **Lofi** (should have been "Rainy afternoon alone"): "Thank you for watching!"
- **Funk** (should have been "Get up on your feet and move"): "Thanks for watching!"

The model learned to generate YouTube outros instead of lyrics.

One genre actually got *perfect* results: **reggae** (WER 0.00, every word correct). And **new wave** was nearly perfect (WER 0.06). But those are two successes out of 30.
...
In every single one, the LoRA+ASHR attention map shows the same pattern: **one or two extremely bright horizontal bands, with everything else completely dark**. Every audio frame, at every point in time, attends to the exact same one or two tokens. Usually the structural tag `[Verse]` or a newline character.
...
Compare this to the Baseline and LoRA panels in the same plots: their attention isn't perfectly diagonal, but there's temporal variation. Different frames attend to different tokens to different degrees. That imperfect, messy pattern is actually doing useful work. The ASHR penalty nuked it.
...
2. **Visual similarity in attention maps can hide functional differences.** The Baseline and LoRA maps often look very similar when averaged across layers and heads, but the outputs are dramatically different (silence vs. three lines of correct lyrics). The meaningful changes happen at individual layers and heads.

3. **The conditioning pathway is a single point of failure.** All lyric information reaches generation through cross-attention. If cross-attention fails, lyrics fail. This makes it both the most impactful intervention point and the most fragile one — as the ASHR experiment demonstrated.

### About Negative Results

The ASHR experiment is a failure in terms of improving lyrics, but it's the most informative result of the entire project:
...
|-----------|-------------|-----|-----|---------|
| Baseline ACE-Step v1.5 | Nothing | 56.42% | 49.32% | Lyrics bad in most genres |
| + LoRA (r=8, 100 epochs) | Fine-tuned on clean vocal audio with descriptive prompts (no word lyrics) | 49.40% | 41.19% | Improved vocal clarity → improved lyric intelligibility as a side effect. Regressions in some genres. |
| + LoRA + ASHR (λ=0.02, 50 epochs) | Added entropy penalty on cross-attention | 86.46% | 74.89% | Catastrophic failure. Attention collapse. YouTube outro generator. |
...
- **ASHR partial recovery** — with real lyrics to align *to*, the entropy penalty shouldn't collapse to padding tokens
...
I ran two new training configurations on the corrected data — LoRA with lyrics (rank 8, alpha 16, lr 1e-4, 55 epochs) and LoRA with lyrics + ASHR (λ=0.02, 55 epochs) — then re-evaluated all five models across the same 30-genre test set under identical conditions (same prompts, same random seeds) for a fair comparison.

| Model | WER (%) ↓ | CER (%) ↓ | AAS (DiT) ↑ | AAS (LM) ↑ |
|-------|-----------|-----------|-------------|-------------|
| Baseline ACE-Step v1.5 | 61.03 | 51.91 | 0.118 | 0.181 |
| + LoRA (no lyrics) | **46.51** | 39.08 | 0.150 | 0.181 |
| + LoRA + ASHR (no lyrics) | 89.21 | 77.73 | 0.060 | 0.149 |
| + LoRA with lyrics | 47.14 | **38.06** | 0.133 | **0.197** |
| + LoRA with lyrics + ASHR | 64.23 | 47.52 | **0.166** | 0.175 |

*All five models evaluated on 30 genres with identical random seeds for fair comparison. Numbers for baseline/LoRA/LoRA+ASHR differ slightly from the Part 1 tables because this is a fresh evaluation run — the relative ordering and conclusions are the same.*
...
The averages are hiding something important. When you break down the per-genre results, the two LoRA models have almost perfectly **complementary** strengths — they each win roughly 10 out of 30 genres, but *different* genres.
...
These are genres where the Part 1 LoRA completely failed — producing silence, instrumental-only output, or degenerate repetitions like "So So So So So." The lyrics-trained model rescued them. Soul went from 72% of words wrong to literally perfect. Disco went from complete silence to a recognizable dance track with nearly all lyrics correct.
...
The rock result deserves a moment. The no-lyrics LoRA produced reasonably clear singing: "Standing on the edge of town, Staring for the sun to rise, Holding on to what is mine, Staring for the open sky" — WER 0.17, minor word substitutions but clearly the right lyrics. The lyrics-trained LoRA produced "CBD Black Bloomberg Sour Full Appearance" — complete gibberish that sounds like a neural network having a seizure. Same genre, same prompt, same seed, different adapter.
...
|-----------|-----------|----------|
| Baseline | ~4 | EDM (0.00), R&B (0.14), Singer-songwriter (0.11) |
| LoRA (no lyrics) | ~10 | Pop (0.00), Synthwave (0.00), Bossa Nova (0.00), Rock (0.17), New Wave (0.00) |
| LoRA with lyrics | ~10 | Soul (0.00), Gospel (0.00), Disco (0.13), Lullaby (0.18), Jazz (0.21) |
| LoRA+lyrics+ASHR | ~3 | Punk (0.04), Indie (0.05), Celtic (0.28) |
| LoRA+ASHR (no lyrics) | ~1 | Latin Pop (0.06) — one of its rare non-catastrophic results |

No single model wins everywhere. If you could pick the best adapter per genre, the hypothetical "oracle ensemble" would dramatically outperform any individual model. This suggests the path forward isn't "one LoRA to rule them all" but rather a mixture or routing system that selects the right adapter based on genre characteristics.
...
| LoRA+ASHR | "And I visit the answer / And I meet the sea / And we see we'll start to grow" | 0.76 |
...
| LoRA+lyrics+ASHR | "Take my hand and then walk with me / Down the road we used to know / Every river meets the sea / It's the sea that will start to grow" | 0.32 |

A clean progression from silence → partial → perfect. The lyrics-trained model nailed every word. Even the ASHR variant got close.
...
| LoRA+ASHR (no lyrics) | "Turning down the wall tonight, nothing left for us to lose / Standing up to join some fight, we will never pay our dues" | 0.22 |
...
| **LoRA+lyrics+ASHR** | **"Burning down the walls tonight / Nothing left for us to lose / Standing up to join the fight / We will never pay your dues"** | **0.04** |

Nearly perfect — only "your" instead of "our." Punk's strong rhythmic delivery aligns well with the forced sharp attention from the ASHR penalty. When the music has a clear, percussive beat and the words land on strong rhythmic positions, sharpened attention is an asset rather than a liability.
...
| LoRA+ASHR (no lyrics) | 0.55 |
| LoRA with lyrics | 0.05 |
| LoRA+lyrics+ASHR | **2.10** (!!!) |

The LoRA (no lyrics) model achieved a perfect score on the evaluation seed (43). To sanity-check this, I ran three additional seeds (44, 45, 46) — two were also perfect, one had minor substitutions. The mean WER across four seeds was 0.075, so the strong result holds up but isn't universally zero. The lyrics+ASHR variant produced an explosive hallucination — WER exceeding 2.0 means it generated more than twice the words in the ground truth, none of them correct: *"Like and thinning will be fine / Dog and dog's in line / Chin-na-ta-tine / Lines on your door / Turn it this time / First off, every happy, happy day..."* — a word salad roughly three times longer than the original lyrics. The sharp attention penalty locked the model into lyric tokens but it couldn't decode them, producing an uncontrollable stream of wrong words.

---

## LoRA + ASHR With Real Lyrics: The Partial Redemption

Remember, in Part 1, the attention sharpness regularization (ASHR) with no lyrics was our most catastrophic failure — WER rocketed to 89%, the model generated YouTube outros instead of music, and the attention collapsed to single tokens. We hypothesized that ASHR failed partly because there were no real lyrics to align *to*.

The results partially confirm this hypothesis.

| Metric | LoRA+ASHR (no lyrics) | LoRA+ASHR (with lyrics) | Change |
|--------|---------------------|----------------------|--------|
| WER | 89.21% | 64.23% | -25 points |
| CER | 77.73% | 47.52% | -30 points |
| AAS (DiT) | 0.060 | **0.166** | Nearly 3x improvement |
| Silence/gibberish | 22 of 30 genres | 12 of 30 genres | Fewer failures |

The with-lyrics ASHR model is still worse than the simpler LoRA variants on average WER, but it's no longer catastrophic. More importantly, it achieves the **highest attention alignment score of any model** (0.166) — its cross-attention patterns are more structured and temporally organized than any other variant, including the baseline.

### Why Best Alignment ≠ Best Lyrics

Here's what the attention metrics from the codex analysis pipeline reveal:

| Metric | Baseline | LoRA | LoRA+ASHR | LoRA+lyrics | LoRA+lyrics+ASHR |
|--------|----------|------|----------|-------------|-----------------|
...
*Lyric Mass = fraction of cross-attention going to lyric tokens vs. caption/timbre tokens. Entropy = how spread the attention is across tokens (lower = sharper). Data from the codex analysis pipeline using lyric-only renormalization.*

The lora+lyrics+ASHR variant devotes **72-84% of its cross-attention to lyric tokens** — dramatically more than any other model (30-64%). And its entropy is consistently the lowest, meaning its attention is the sharpest. The ASHR penalty with real lyrics causes the model to:

1. **Focus on lyrics, not caption.** While other models split attention roughly 40/60 between lyrics and caption, the ASHR+lyrics model overwhelmingly attends to the lyric tokens.
...
The cases where it DOES work (punk at WER 0.04, indie at 0.05, tropical house at 0.15) tend to be genres with clear, rhythmic vocal delivery where the phonetic mapping is simpler. The cases where it fails catastrophically (pop at WER 2.10) tend to be genres requiring more melodic, flowing vocals where the rigid attention pattern fights against the music's natural phrasing.

### The ASHR Controlled Experiment

Putting the four LoRA variants in a 2×2 grid reveals the cleanest controlled experiment in this project:

| | No ASHR (λ=0) | ASHR (λ=0.02) |
|---|---|---|
| **No lyrics** | LoRA: WER 46.51% (improved) | LoRA+ASHR: WER 89.21% (catastrophic) |
| **With lyrics** | LoRA+lyrics: WER 47.14% (improved) | LoRA+lyrics+ASHR: WER 64.23% (partial recovery) |

Without lyrics, ASHR causes attention collapse — the model attends to padding tokens because that's the cheapest way to minimize entropy. With lyrics, ASHR forces the model to attend sharply to real word tokens, which is actually useful — but λ=0.02 is still too aggressive for general use.
...
To dig deeper, we generated per-head attention maps for all five model variants across four genres (folk, pop, jazz, R&B) using the improved codex analysis pipeline. This pipeline uses deterministic extraction at the schedule-consistent timestep, lyric-only renormalization to isolate the lyric-audio relationship, and shared color scales for fair comparison across variants.

### Folk — Per-Head Across All Five Variants

**Baseline:**

![Per-Head Attention — Folk Baseline](figures_codex/attention_folk_baseline_per_head.png)

**LoRA (no lyrics):**

![Per-Head Attention — Folk LoRA](figures_codex/attention_folk_lora_per_head.png)

**LoRA+ASHR (no lyrics):**

![Per-Head Attention — Folk LoRA+ASHR](figures_codex/attention_folk_lora_asr_per_head.png)

**LoRA with lyrics:**

![Per-Head Attention — Folk LoRA with Lyrics](figures_codex/attention_folk_lora_with_lyrics_per_head.png)

**LoRA with lyrics+ASHR:**

![Per-Head Attention — Folk LoRA with Lyrics+ASHR](figures_codex/attention_folk_lora_with_lyrics_asr_per_head.png)
...
But the subtle differences between variants matter. The LoRA with lyrics folk per-head map shows slightly more variation in band structure compared to the no-lyrics LoRA — some heads have multiple bands or show transitions between tokens. The ASHR+lyrics variant shows the most extreme concentration: nearly all probability mass in a few very sharp bands, consistent with its dramatically higher lyric mass (0.84) and lower entropy (0.48).

### Pop — Per-Head Comparison

**Baseline:**

![Per-Head Attention — Pop Baseline](figures_codex/attention_pop_baseline_per_head.png)

**LoRA (no lyrics)** (WER 0.00 on the evaluation seed; mean 0.075 across 4 seeds — consistently strong):

![Per-Head Attention — Pop LoRA](figures_codex/attention_pop_lora_per_head.png)

**LoRA with lyrics:**

![Per-Head Attention — Pop LoRA with Lyrics](figures_codex/attention_pop_lora_with_lyrics_per_head.png)

**LoRA with lyrics+ASHR** (WER 2.10 — catastrophic hallucination):

![Per-Head Attention — Pop LoRA with Lyrics+ASHR](figures_codex/attention_pop_lora_with_lyrics_asr_per_head.png)

The pop per-head maps show a stark progression. The LoRA variant (WER 0.00 on the eval seed, mean 0.075 across 4 seeds) shows structured head behavior — a few heads carry temporal patterns while others stay collapsed. The lyrics+ASHR variant, which hallucinated catastrophically, shows the extreme version: attention mass concentrated in sharp bands with barely any temporal variation. The model was *staring at the lyrics* but couldn't decode them.

### Jazz and R&B — Per-Head Views

**Jazz Baseline:**

![Per-Head Attention — Jazz Baseline](figures_codex/attention_jazz_baseline_per_head.png)

**Jazz LoRA with Lyrics:**

![Per-Head Attention — Jazz LoRA with Lyrics](figures_codex/attention_jazz_lora_with_lyrics_per_head.png)

**R&B Baseline:**

![Per-Head Attention — R&B Baseline](figures_codex/attention_rnb_baseline_per_head.png)

**R&B LoRA (no lyrics):**

![Per-Head Attention — R&B LoRA](figures_codex/attention_rnb_lora_per_head.png)

**R&B LoRA with Lyrics:**

![Per-Head Attention — R&B LoRA with Lyrics](figures_codex/attention_rnb_lora_with_lyrics_per_head.png)

### The Sparse Alignment Signal

The per-layer-head statistics quantify what the figures show qualitatively. For each of the 384 layer-head combinations (24 layers × 16 heads), the codex pipeline computed a "diagonal score" measuring how well the attention path follows the temporal order of lyrics. Here are the best individual heads for folk:
...
| LoRA+lyrics+ASHR | Head 9 | Layer 5 | 0.165 | 0.82 |
| LoRA (no lyrics) | Head 4 | Layer 15 | 0.146 | — |
| LoRA+ASHR (no lyrics) | — | — | 0.090 | — |

The lyrics-trained model has the highest single-head diagonal score for folk (0.224, with 0.77 correlation to temporal order). And something interesting: this best head lives at **Layer 3** — one of the earliest DiT layers. In the baseline, the best alignment head is at Layer 15, deep in the network. Lyrics training appears to have shifted alignment responsibility toward earlier layers. This could be significant — early-layer alignment means the temporal structure is established before the deeper layers handle higher-level musical decisions.

Across all four genres:

| Genre | Baseline Best | LoRA Best | LoRA+lyrics Best | LoRA+lyrics+ASHR Best |
|-------|-------------|-----------|-----------------|---------------------|
...
![Cross-Attention Maps — Folk (5 variants)](figures/attention_folk.png)

The **LoRA with lyrics** panel (fourth) shows a different attention distribution compared to LoRA without lyrics. There's more lyric mass (the lyric portion of the heatmap is brighter relative to the summary), consistent with the codex metrics showing lyric mass of 0.64 vs 0.48. The **LoRA+lyrics+ASHR** panel (fifth) is the most striking — overwhelmingly concentrated on lyric tokens with sharp, distinct bands. This is what lyric mass of 0.84 looks like.

### Jazz — Five-Way Comparison

![Cross-Attention Maps — Jazz (5 variants)](figures/attention_jazz.png)

The lyrics-trained model improved jazz WER from 0.42 to 0.21. The heatmap shows why — the fourth panel has more distributed attention across lyric tokens compared to the third panel (LoRA+ASHR, which is nearly uniform). The fifth panel (lyrics+ASHR) shows extreme lyric focus but in a less structured way.

### R&B — Five-Way Comparison

![Cross-Attention Maps — R&B (5 variants)](figures/attention_rnb.png)
...
![Cross-Attention Maps — Pop (5 variants)](figures/attention_pop.png)

The second panel (LoRA, WER 0.00) and fourth panel (LoRA with lyrics, WER 0.05) look qualitatively similar and both produce excellent results. The fifth panel (lyrics+ASHR, WER 2.10) shows dramatically concentrated attention — the model is staring intensely at the lyrics but generating a word salad.
...
| + LoRA (no lyrics) | Clean vocals, `[verse]` only | **46.51** | 39.08 | 0.150 | Improved vocal clarity → better lyric intelligibility. Best at ~10 genres. |
| + LoRA + ASHR (no lyrics) | + entropy penalty | 89.21 | 77.73 | 0.060 | Catastrophic attention collapse. YouTube outro generator. |
| + LoRA with lyrics | Clean vocals + real lyrics | 47.14 | **38.06** | 0.133 | Nearly identical average, but wins ~10 *different* genres. Complementary to no-lyrics LoRA. |
| + LoRA with lyrics + ASHR | + entropy penalty | 64.23 | 47.52 | **0.166** | No longer catastrophic. Best attention alignment. Sparse genre wins (punk, indie). |
...
The 2×2 ASHR experiment provides the cleanest controlled result:
...
Without lyrics, ASHR causes attention collapse to padding tokens. With lyrics, ASHR forces attention to real word tokens — useful in principle, still too aggressive at λ=0.02. **Entropy regularization on cross-attention requires meaningful conditioning content to work.**

### 4. Attention Alignment ≠ Output Quality

The lyrics+ASHR model has the best AAS of any variant (0.166) yet worse WER (64.23%) than both simpler LoRA models (~47%). This definitively shows that structured attention patterns don't automatically produce correct lyrics.
...
The hypothetical "oracle" that picks the best model per genre would dramatically outperform any individual variant. The practical implication: build a routing mechanism that selects the right adapter based on genre or prompt characteristics, or explore weight-space merging techniques (adapter arithmetic at different scales) to capture complementary strengths in a single model.

---

## Listen For Yourself: Audio Examples

Numbers and heatmaps only tell half the story. Below are generated audio samples for two genres — folk and pop — across all five model variants, using the same prompts, lyrics, and random seeds from the evaluation. Put on headphones and try to follow the lyrics as you listen.

**Ground truth lyrics (Folk):** *"Walking down the morning road / Sunlight breaking through the trees / Every step a story told / Carried softly on the breeze"*

| Variant | WER | Audio |
|---------|-----|-------|
| Baseline | 1.00 (silence) | [folk_baseline.flac](audio_examples/folk_baseline.flac) |
| LoRA (no lyrics) | 0.40 | [folk_lora.flac](audio_examples/folk_lora.flac) |
| LoRA+ASHR (no lyrics) | 0.65 | [folk_lora_ashr.flac](audio_examples/folk_lora_ashr.flac) |
| LoRA with lyrics | 0.35 | [folk_lora_with_lyrics.flac](audio_examples/folk_lora_with_lyrics.flac) |
| LoRA with lyrics+ASHR | 0.55 | [folk_lora_with_lyrics_ashr.flac](audio_examples/folk_lora_with_lyrics_ashr.flac) |
...
| Variant | WER | Audio |
|---------|-----|-------|
| Baseline | 0.85 | [pop_baseline.flac](audio_examples/pop_baseline.flac) |
| LoRA (no lyrics) | 0.00 (perfect) | [pop_lora.flac](audio_examples/pop_lora.flac) |
| LoRA+ASHR (no lyrics) | 0.55 | [pop_lora_ashr.flac](audio_examples/pop_lora_ashr.flac) |
| LoRA with lyrics | 0.05 | [pop_lora_with_lyrics.flac](audio_examples/pop_lora_with_lyrics.flac) |
| LoRA with lyrics+ASHR | 2.10 (hallucination) | [pop_lora_with_lyrics_ashr.flac](audio_examples/pop_lora_with_lyrics_ashr.flac) |

The folk examples demonstrate the "rescue" effect — the baseline produces silence, while both LoRA variants generate audible vocals. The pop examples show the opposite extreme: LoRA (no lyrics) achieves a near-perfect transcription (WER 0.00 on this seed, mean 0.075 across 4 seeds), but the lyrics+ASHR variant hallucinates an uncontrollable stream of wrong words despite staring intensely at the correct lyric tokens.

---

*This started as a random experiment on a Friday night — throw some clean vocal data at a music generation model and see what happens. A preprocessing bug turned it into a natural ablation study. A failed entropy penalty taught us more about cross-attention dynamics than any of the successes. And fixing the bug revealed the most surprising result of all: adding lyrics doesn't straightforwardly improve things — it shifts the distribution of where the model succeeds and fails, creating a complementary adapter rather than a strictly better one.*

*The biggest surprise? Two LoRA adapters with identical architecture, trained on the same audio with different lyrics fields, produce models that win on almost entirely different genre sets. Rank 8 is apparently expressive enough to encode qualitatively different behaviors depending on the training signal. And the best attention alignment doesn't produce the best lyrics — because knowing where to look is only half the battle.*

*If I were to continue this (and I probably will), the next step would be: (a) a simple routing mechanism that picks the right adapter per genre, (b) merging the two adapters through weight arithmetic (adapter A + adapter B at different scales) to capture both skills simultaneously, or (c) training a single adapter on a mixture of lyric-aligned and caption-only samples. The model has shown it can learn both vocal clarity and lyric alignment — the question is whether it can learn both at the same time, in the same rank-8 subspace.*
